# Fedora 35 Container Image for Ansible Testing

This Dockerfile builds a Fedora 35 based container, capable of using `systemd`, mainly for Ansible role testing.
...
2. Clone the repository: 
   ...
   ```shell
   git clone https://github.com/chzerv/fedora35-systemd-ansible.git
   ```
3. `cd` into the directory and run `docker build -t fedora35-systemd-ansible .`
...
2. Pull this image from _Docker hub_:
    ...
    ```shell
    docker pull chzerv/fedora35-systemd-ansible:latest
    ```
...
```shell
docker run -d --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro fedora35-systemd-ansible:latest ansible --version
```
...
platforms:
  - name: instance
    image: "chzerv/${IMAGE:-fedora35}-systemd-ansible:latest"
